...
$app->get('/leagues/{league_name}/{club_name}[/]', function ($request, $response, $args) {
    $league_name = mysqli_real_escape_string($GLOBALS['conn'], $args['league_name']);
    $league_name = str_replace('-', ' ', $league_name);
    
    $club_name = mysqli_real_escape_string($GLOBALS['conn'], $args['club_name']);
    ...
    if (mysqli_stmt_prepare($stmt, $sql)) {
        ...
        $sql_2 = "SELECT NULL FROM `leagues` WHERE `name` = ?;";
        $stmt_2 = mysqli_stmt_init($GLOBALS['conn']);
        if (mysqli_stmt_prepare($stmt_2, $sql_2)) {
            mysqli_stmt_bind_param($stmt_2, 's', $league_name);
            mysqli_stmt_execute($stmt_2);
            $result_check_2 = mysqli_num_rows(mysqli_stmt_get_result($stmt_2));
            if ($result_check_2 == 0) {
                http_response_code(404);
                die();
            }
        ...
        if ($row = mysqli_fetch_assoc($result)) {
           ?>
            <div class="profile_parent">
                <div class="profile">
                    <div class="left">
                        <img src="/phone/images/clubs/<?php echo $row['img']; ?>" alt="<?php echo $row['name']; ?>" class="league_logo">
                    </div>
                    <div class="right">
    ...
    <?php
            $id = $row['id'];
            $sql = "SELECT * FROM `products` WHERE `club_id` = '" . $id . "';";
