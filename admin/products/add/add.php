...
if (isset($_SERVER['HTTP_REFERER'])) {
    if (strpos($_SERVER['HTTP_REFERER'], '/admin/products/add') <= 0){
        http_response_code(404);
        die;
    }
}
...
if (!isset($_POST['product_name']) || !isset($_POST['product_price']) || !isset($_POST['discount']) || !isset($_POST['phones']) || !isset($_POST['variants']) || !isset($_POST['league']) || !isset($_POST['club']) || !isset($_POST['collection']) || !isset($_POST['mockups']) || !isset($_POST['mockups_92']) || !isset($_POST['mockup_156'])) {
    http_response_code(404);
    die;
}
...
$sql = "SELECT null FROM `products` WHERE `name` = ?";
...
/*
check if name is valid for file name
*/
...
$GLOBALS['product_id'] = null;

$sql = "INSERT INTO `products` (`name`, `price`, `club_id`, `league_id`, `collection_id`, `file`, `discount`) VALUES(?, ?, ?, ?, ?, ?, ?)";
...
if (mysqli_stmt_prepare($stmt, $sql)) {
    ...
    mysqli_stmt_bind_param($stmt, 'siiiisi', $product_name, $product_price, $GLOBALS['club_id'], $GLOBALS['league_id'], $GLOBALS['collection_id'], $file_name, $discount);
        ...
        if (mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($row = mysqli_fetch_assoc($result)) {
                ...
                if (mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    $GLOBALS['file_name'] .= '-'.$row['id'].'.json';
                    

                    mysqli_stmt_bind_param($stmt, 'si', $GLOBALS['file_name'], $row['id']);
                    mysqli_stmt_execute($stmt);
...
foreach ($json_variants as $value) {
    ...
    $filePath = '../../../assets/print/' . $value['name'] . '-' . $GLOBALS['product_id'] .'.png';
    ...
    array_push($new_json_variants, [
        'img' => $value['name'] . '-' . $GLOBALS['product_id'] .'.png',
        'name' => $value['name']
    ]);
...
foreach ($json_mockups as $value) {
    ...
    if ($index === 0) {
        ...
        $imgData = base64_decode($imgData);
        // Path where the image is going to be saved
        $filePath = '../../../assets/mockups/156/'. $product_name_new . "-" .$index. "-" .$GLOBALS['product_id'] .'.png';
        // Write $imgData into the image file
        $file = fopen($filePath, 'w');
        fwrite($file, $imgData);
        fclose($file);
    ...
    $imgData = base64_decode($imgData);
    // Path where the image is going to be saved
    $filePath = '../../../assets/mockups/350/'. $product_name_new . '-' . $index . "-" .$GLOBALS['product_id'] .'.png';
    // Write $imgData into the image file
    $file = fopen($filePath, 'w');
    fwrite($file, $imgData);
    fclose($file);

    array_push($new_json_mockups, $product_name_new . '-' . $index . "-" .$GLOBALS['product_id'] .'.png');
...
foreach ($json_mockups_92 as $value) {
    ...
    $imgData = base64_decode($imgData);
    // Path where the image is going to be saved
    $filePath = '../../../assets/mockups/92/'. $product_name_new . '-' . $i . "-" .$GLOBALS['product_id'] .'.png';
    // Write $imgData into the image file
    $file = fopen($filePath, 'w');
    fwrite($file, $imgData);
    fclose($file);
...
var elt = document.createElement("li")
elt.appendChild(document.createTextNode("Error, unable to generate file."))
document.querySelector(".errorAll").appendChild(elt)
');
...
echo trim('
    document.querySelector(".successAll").textContent = "Product added Successfully!"
');