...
if (!preg_match('/^[a-zA-Z0-9 ]*$/i', $name)) {
    $GLOBALS['script'] .= '
        document.querySelector("#name").classList.add("invalid")
        document.querySelector("#name").nextElementSibling.textContent = "Invalid collection name."
    ';
}
...
$sql = "SELECT null FROM `collections` WHERE `name` = ?;";
...
if ($result_check > 0) {
    echo trim('
        document.querySelector("#name").classList.add("invalid")
        document.querySelector("#name").nextElementSibling.textContent = "A collection with the same name exist! Please try a different name."
    ');
    die;
}
...
$sql = "INSERT INTO `collections` (`name`, `img`) VALUES(?, ?)";
...
if (mysqli_stmt_affected_rows($stmt) > 0) {
    echo trim('document.querySelector("#addLeague .successAll").textContent = "Collection added successfully"');
    die;
}else {
    echo trim('document.querySelector("#addLeague .errorAll").textContent = "Failed to add Collection. Please try again later."');
    die;
}