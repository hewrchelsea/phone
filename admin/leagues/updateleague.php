...
if (strlen($id) > 11 || $id < 0) {
    echo trim('
        var elt = document.createElement("li")
        elt.appendChild(document.createTextNode("Couldn\'t update league, because the league wasn\'t found!"))
        document.querySelector(".update_errorAll").appendChild(elt)
    ');
    die;
}
...
if (!preg_match('/^[a-zA-Z0-9 ]*$/i', $name)) {
    $GLOBALS['script'] .= '
        document.querySelector("#update_name").classList.add("invalid")
        document.querySelector("#update_name").nextElementSibling.textContent = "Invalid league name."
    ';
    die;
}
...
$sql = "SELECT null FROM `leagues` WHERE `id` = ?;";
...
if ($result_check === 0) {
    echo '
        var elt = document.createElement("li")
        elt.appendChild(document.createTextNode("The league you are trying to update doesn\'t exist"))
        document.querySelector(".update_errorAll").appendChild(elt)
    ';
    die;
}
...
$sql = "SELECT null FROM `leagues` WHERE `name` = ? AND id != ?;";
...
if ($result_check > 0) {
    $GLOBALS['script'] .= '
        document.querySelector("#update_name").classList.add("invalid")
        document.querySelector("#update_name").nextElementSibling.textContent = "A league with the same name exist! Please try a different name."
    ';
    die;
}
...
$sql = "UPDATE `leagues` SET `name` = ?, `img` = ? WHERE `id` = ?;";
$stmt = mysqli_stmt_init($conn);
...
echo '
    var elt = document.createElement("li")
    elt.appendChild(document.createTextNode("League updated successfully."))
